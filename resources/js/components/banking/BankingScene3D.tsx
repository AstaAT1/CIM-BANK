...
export default function BankingScene3D() {
    ...
    useEffect(() => {
        ...
        const navyM = new THREE.MeshPhysicalMaterial({
            color: 0x082f54,
            metalness: 0.9,
            roughness: 0.1,
            clearcoat: 1,
        ...
        const coinEdgeM = new THREE.MeshStandardMaterial({
            color: 0xc89530,
            metalness: 0.95,
            roughness: 0.15,
        });
        ...
        const cGeo = new THREE.ExtrudeGeometry(makeCardShape(3.2, 2.0, 0.15), {
            depth: 0.04,
            bevelEnabled: true,
        ...
        for (let i = 0; i < 3; i++) {
            const cl = new THREE.Mesh(
                new THREE.BoxGeometry(0.35, 0.008, 0.005),
        ...
        // CIM text line
        const cimLine = new THREE.Mesh(
            new THREE.BoxGeometry(0.6, 0.06, 0.005),
            goldM.clone(),
        );
        ...
        const stripe = new THREE.Mesh(
            new THREE.BoxGeometry(3.0, 0.06, 0.01),
            goldM.clone(),
        );
        ...
        const c2Geo = new THREE.ExtrudeGeometry(makeCardShape(2.4, 1.5, 0.12), {
            depth: 0.03,
            bevelEnabled: true,
        ...
        const chip2 = new THREE.Mesh(
            new THREE.BoxGeometry(0.3, 0.22, 0.015),
            goldM.clone(),
        );
        chip2.position.set(-0.6, 0.25, 0.04);
        card2.add(chip2);
        const stripe2 = new THREE.Mesh(
            new THREE.BoxGeometry(2.2, 0.04, 0.008),
            goldM.clone(),
        );
        ...
        ringDefs.forEach(([r, t, pos, rot], i) => {
            ...
            const mesh = new THREE.Mesh(
                new THREE.TorusGeometry(r, t, 24, 128),
                mat,
            );
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const trail = new THREE.Mesh(
                new THREE.TorusGeometry(7 + i * 1.5, 0.008, 8, 128),
                trailMat,
            );
            trail.position.set(
                (Math.random() - 0.5) * 4,
