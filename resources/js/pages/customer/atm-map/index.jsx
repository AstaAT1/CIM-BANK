...
function cashFill(atm) {
    const m = asNumber(atm?.max_capacity);
    return m > 0
        ? Math.max(0, Math.min(100, (asNumber(atm.current_cash) / m) * 100))
        : 0;
}
...
function distanceKm(from, to) {
    ...
    const R = 6371,
        r = (v) => (v * Math.PI) / 180;
    const dLat = r(to[0] - from[0]),
        dLng = r(to[1] - from[1]);
    const a =
        Math.sin(dLat / 2) ** 2 +
        Math.cos(r(from[0])) * Math.cos(r(to[0])) * Math.sin(dLng / 2) ** 2;
...
function MapEffects({ locateRequest, selectedPosition, userLocation, mapRef }) {
    ...
    useEffect(() => {
        if (selectedPosition && userLocation) {
            map.fitBounds([userLocation, selectedPosition], {
                animate: true,
                duration: 0.85,
                padding: [90, 90],
            });
...
function CashBar({ atm }) {
    ...
    const color =
        fill >= 40
            ? STATUS.active.color
            : fill >= 15
              ? STATUS.low_cash.color
              : STATUS.empty.color;
    return (
        <div>
            <div className="mb-1.5 flex justify-between text-[11px]">
                <span className="font-medium text-slate-400">Cash level</span>
...
function FloatingSidebar({
    ...
    return (
        <div className="absolute top-0 left-0 z-[900] flex h-full">
                    ...
                    <div className="flex h-full flex-col overflow-hidden">
                        ...
                        <div className="shrink-0 border-b border-white/8 p-4">
                            <div className="mb-4 flex items-center gap-3">
                                <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[#D4A23C] text-xs font-black text-[#061F39]">
                                    CIM
                                </div>
                                <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    className="h-9 w-full rounded-lg pr-3 pl-8 text-sm text-white placeholder-slate-500 transition outline-none"
                                    style={{
                        ...
                        {/* filters */}
                        <div className="shrink-0 space-y-1 border-b border-white/8 px-3 py-3">
                            {FILTERS.map((f) => {
            ...
            <button
                onClick={onToggle}
                className="absolute top-1/2 flex -translate-y-1/2 items-center justify-center rounded-r-lg transition-all duration-200 hover:scale-105"
                style={{
        ...
        >
            ...
            <div className="flex items-center gap-2">
                <div
                    ...
                    style={{
                        background: `${STATUS.active.color}18`,
                        color: STATUS.active.color,
                        border: `1px solid ${STATUS.active.color}33`,
                    }}
                >
                    <span
                        className="size-1.5 animate-pulse rounded-full"
                        style={{ background: STATUS.active.color }}
                    />
        ...
        <div className="absolute right-4 bottom-8 z-[800] flex flex-col gap-2">
            {[
                ...
                <button
                    ...
                    onClick={action}
                    title={tip}
                    className="flex size-10 items-center justify-center rounded-xl transition-all duration-150 hover:scale-105 hover:brightness-110"
                    style={{
...
function SelectedAtmPanel({
    ...
    return (
        <div
            className="absolute right-0 bottom-0 z-[850] flex flex-col overflow-hidden transition-all duration-300"
            style={{
                width: 360,
                ...
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <h3 className="mt-0.5 text-base leading-tight font-bold text-white">
                                {atm.name}
                            </h3>
                    ...
                    <div className="flex gap-1.5">
                        <button
                            ...
                            className="flex size-7 items-center justify-center rounded-lg text-slate-400 transition hover:text-white"
                ...
                <div className="flex-1 space-y-4 overflow-y-auto p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {[
                            ...
                            {
                                label: 'Distance',
                                val: formatDistance(routeDistance),
                            },
                        ].map(({ label, val }) => (
                            <div
                                key={label}
                    ...
                    <div
                        ...
                        style={{
                            ...
                            border: `1px solid ${CIM.accent}22`,
                        }}
                    >
                        <Route
                    ...
                    >
                        <div className="flex items-center gap-2">
                            <div
                                ...
                                style={{ background: `${CIM.accent}18` }}
                            >
                                <Banknote
                                    className="size-4"
                                    style={{ color: CIM.accent }}
                        ...
                        <div>
                            ...
                            <select
                                ...
                                className="h-10 w-full rounded-lg px-3 text-sm text-white transition outline-none disabled:opacity-50"
                                style={{
                            ...
                            {selectedAccount && (
                                <p className="mt-1 text-[10px] text-slate-500">
                                    ...
                                    <span className="font-semibold text-slate-300">
                                        {formatMad(selectedAccount.balance)}
                                    </span>
                                </p>
                            )}
                        ...
                        <div>
                            ...
                            >
                                <input
                                    ...
                                    className="h-10 flex-1 bg-transparent px-3 text-base font-bold text-white placeholder-slate-600 outline-none disabled:opacity-50"
                                />
                                <span
                                    className="flex items-center border-l px-3 text-xs font-bold text-slate-500"
                                    style={{
                                        background: 'rgba(255,255,255,0.04)',
                        ...
                        <button
                            ...
                            className="flex h-10 w-full items-center justify-center gap-2 rounded-lg text-sm font-bold transition-all duration-200 hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
...
export default function AtmMapIndex({
    ...
    const filteredAtms = useMemo(() => {
        ...
        return atms.filter((atm) => {
            ...
            const matchSearch =
                !q ||
                [atm.name, atm.area, atm.code, atm.address]
                    .filter(Boolean)
                    .join(' ')
                    .toLowerCase()
                    .includes(q);
        ...
        () =>
            FILTERS.reduce((acc, f) => {
                acc[f.key] =
                    f.key === 'all'
                        ? atms.length
                        : atms.filter((a) => a.status === f.key).length;
                return acc;
            }, {}),
        [atms],
    );
    ...
    const locationLabel =
        locationSource === 'browser' ? 'your location' : 'LionsGeek, Ain Sebaa';
    ...
    useEffect(() => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            () => {
                setUserLocation(LIONSGEEK_LOCATION);
    ...
    const userIcon = useMemo(
        () => (canRenderMap ? buildUserIcon() : null),
        [canRenderMap],
    );
    ...
    return (
        <>
            ...
            <main
                ...
                style={{ height: '100vh', background: '#060f1e' }}
            >
                {canRenderMap ? (
                    <MapContainer
                    ...
                    >
                        ...
                        <InnerControls
                            onLocate={() => setLocateRequest((c) => c + 1)}
                        />
                    </MapContainer>
                ) : (
